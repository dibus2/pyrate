...
def writeLatexOutput(RunSettings,FinalRGE,model,ToCalculate):
		...
		\section{{Summary}}
			...
			\\begin{{center}} model: {0} \end{{center}}
			Settings :
			\\begin{{verbatim}}
			{1}
			\\end{{verbatim}}
...
def writeGaugeCouplings(LatexText,FinalRGE,model,RunSettings):
	...
	LatexText += '\section{Gauge Couplings}'
	LatexText += r"""
		\begin{align*}
...
def writeQuarticCouplings(LatexText,FinalRGE,model,RunSettings,counter):
	...
	LatexText += '\section{Quartic Coupling}'
	LatexText += r"""
		\begin{align*}
	...
	for lbda in model.LbdToCalculate.keys() : 
		...
		ExprII = SplitEq(ExprII)
		    ...
		    LatexText = WriteEqs(LatexText,ExprI,ExprII,lbda,RunSettings,combinations = model.Combination['QuarticTerms'][lbda])
                else :
...
def writeFM(LatexText,FinalRGE,model,RunSettings,counter):
	...
	LatexText += '\section{Fermion Mass}'
	LatexText += r"""
		\begin{align*}
	...
	for m in model.FMToCalculate.keys(): 
		...
		ExprII = SplitEq(ExprII)
		    ...
		    LatexText = WriteEqs(LatexText,ExprI,ExprII,m,RunSettings,combinations = model.Combination['FermionMasses'][m])
                else :
...
def writeScalarMass(LatexText,FinalRGE,model,RunSettings,counter):
	...
	for mab in model.ScMToCalculate.keys(): 
		...
		ExprII = SplitEq(ExprII)
    		    ...
    		    LatexText = WriteEqs(LatexText,ExprI,ExprII,mab,RunSettings,combinations = model.Combination['ScalarMasses'][mab])
                else :
...
def writeTrilinear(LatexText,FinalRGE,model,RunSettings,counter):
	...
	for hfTeX in model.TriToCalculate.keys(): 
		...
		ExprII = SplitEq(ExprII)
		    ...
		    LatexText = WriteEqs(LatexText,ExprI,ExprII,hfTeX,RunSettings,combinations = model.Combination['TrilinearTerms'][hfTeX])
                else :
...
def WriteEqs(LatexText,ExprI,ExprII,label,RunSettings,combinations=[]) : 
	if RunSettings['Two-Loop'] :
		if combinations != [] :
			label = latex(Symbol(label))+'+'+'+'.join([latex(Symbol(el[0])*el[1]) for el in combinations if el[1] != 0])
			if label[-1] == '+' :
				label = label[:-1]
		else :
			label = Symbol(label)
		LatexText += r"""
			\subsection{{Evolution of ${0}$}}
				\begin{{align*}}
					\left.(4\pi)^2\beta_{{{0}}}\right|_{{I}}=""".format(latex(label))
		...
		LatexText+=r"""\begin{align*}"""
		LatexText +=r"""
			\left.(4\pi)^4\beta_{{{0}}}\right|_{{II}}=""".format(latex(label))
