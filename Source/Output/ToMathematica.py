...
def ToMathematicaNotation(vall,model,FlagSquare = True):
		...
		vall = vall.replace('conjugate','conj').replace('MatM','MatMul').replace('SP','ScalarProd').replace('Dagger','Adj').replace('transpose','Tp').replace('[ ]','')
	...
	for ll,vall in xpr.items() :
		...
		if ll in model.GetGroupFromName :
			...
			if dims != [1,1] :
				...
				Return.append("{{{},{}}}".format(lltp.replace('_','').replace('\\',''),str(vall).replace('**','^').replace('_','').replace('\\','')))
			if dims != [1,1] :
