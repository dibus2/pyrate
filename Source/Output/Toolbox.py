...
def ExportToMathematica(Name,Xpr,model):
	...
	for iel,el in enumerate(Xpr):
		tp = TranslateToMathematica(el,'{}_{}'.format(Name,model.savesettingsforoutput[0][iel]),model.savesettingsforoutput[0][iel],model)
...
def ExportBetaFunction(name,model):
	...
	def declarematrices(llist,iwritten,Mapping,cpStrucYuk,CodeStr):
		for ii,val in llist:
			if cpStrucYuk[ii][-1] == 'matrix' :
				CodeStr += "\t{} = np.matrix(np.zeros(({},{})))\n".format(maps[ii],cpStrucYuk[ii][0],cpStrucYuk[ii][1])
	...
	if 'Yukawas' in FinalExpr.keys() : 
		Mapping,iwritten,CodeStr = declarematrices(FinalExpr['Yukawas'].items(),iwritten,Mapping,cpStrucYuk,CodeStr)
	if 'FermionMass' in FinalExpr.keys() :
		Mapping,iwritten,CodeStr = declarematrices(FinalExpr['FermionMass'].items(),iwritten,Mapping,cpStrucYuk,CodeStr)
