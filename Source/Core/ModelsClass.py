...
class Model(object):
    ...
    def __init__(self, Settings):
        ...
        self.translation = {_Y: ['s', 'f', 'f'], _Ya: ['s', 'f', 'f'], _C: ['g', 'f'], _Cs: ['g', 's'], _Cg: ['g'],
                            ...
                            'H2abS': ['s', 's'], 'Y2FabS': ['g', 's', 's'], 'Habcd': ['s', 's', 's', 's'],
							'Theta4': ['g', 'g', 's', 's', 's', 's'], 'Theta2': ['g', 's', 's', 's', 's'],
                            'L2abS': ['s', 's'], 'ffabc': ['g'], 'Hab': ['s', 's'], 'Habc': ['s', 's', 's']}
                ...
                try:
                    while not (Found):
                        ...
                        while not (STOP1):
							...
							STOP2 = False
							while not (STOP2):
                                tempF, STOP1, counter, All = GetCombInd(CombInd, STOP1, Return[el][counter], counter,
                                ...
                                if tempF != []:
                                    ...
                                    if all(Valid) and Valid != []:
										...
										STOP2 = True
										LastRun = False
										LHfactor, outparticles = ConstructReturn(el, tempF, Return[el][counter], Return,
                                                                                 ReturnToCalc)
                                        ReturnToCalc[el] = Return[el][counter][0], LHfactor, outparticles
                except IOError:
            ...
            for f in elemfields:
				...
				if f.Cplx and not (Singlet):
					Out.append([IndexedBase(f.RealPart)[[
                        ...
                        for igg, g in enumerate(self.NonUGaugeGroups)]] + f.Coeff * IndexedBase(f.CplxPart)[[
						'i{}{}'.format(IndicesCounters[igg] + 1, igg) if f.Qnb[g[0]] != g[1].Dynksinglet else Integer(0)
            ...
            if StartingV != None:
                ...
                for ix, x in enumerate(argsS[ielem + 1:]):
                    ...
                    if StartingV in FinalV:
                        ...
                        if elem[
                            0] == _C:  # WARNING IF THE C(F) IS OUT OF A SUMMED BLOCK IT IS NOT MULTIPLIED OUT AND THIS WOULD CRASH
                ...
                for ix, x in enumerate(args[ielem + 1:]):
					...
					if StartingV in FinalV:
						...
						InTrace = [iel for iel, el in enumerate(args) if
								   iel in range(ielem + iel4Y, ix + ielem + 1 + 1)]
						if elem[
							0] == _C:  # we have to remove the _C index since it is transformed as a number when called it is not in the trace
                            InTrace.remove(ielem)
						elif x[0] == _C:
                            inTrace.remoce(ix)
                        return InTrace
    ...
    def declarations(self):
        ...
        self.declareSymbol(
            ['_Y', '_Ya', '_C', '_Cs', '_S', '_Ss', '_Cg', '_G', '_mf', '_mfa', '_ms', '_L', '_h', '_Tha', '_Ta', '_Th',
             '_T'])  # the last symbls is for the C2(G)
