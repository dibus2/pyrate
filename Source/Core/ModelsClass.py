...
class Model(object):
    ...
    def __init__(self, Settings):
        ...
        self.TriToCalculate = {}
        self.FMToCalculate = {}
        self.ScalarAnomalousToCalculate = {}
        self.FermionAnomalousToCalculate = {}
        self.MixingTerm = {}
        self.ListLbd = []
        self.ListScM = []
        self.ListFM = []
        ...
        if len(self.UsectorMatrix) > 1:
            ...
            tempG = [['g_U_{}{}'.format(j + 1, i + 1) for i in range(len(self.UsectorMatrix))] for j in
                     range(len(self.UsectorMatrix))]
        ...
        for elem in self.ListFM + self.ListYukawa:
            if elem in ['MatM', 'atM', 'tM', 'M', '\lambda', 'lambda']:
                loggingCritical('Change the name of your coupling {}, it interferes with MatM'.format(elem),
                                verbose=True)
                exit()
        ...
        self.translation = {_Y: ['s', 'f', 'f'], _Ya: ['s', 'f', 'f'], _C: ['g', 'f'], _Cs: ['g', 's'], _Cg: ['g'],
                            _S: ['g'], _Ss: ['g'], _G: ['g'],
                            _mfa: ['f', 'f'], _mf: ['f', 'f'], _ms: ['s', 's'], _h: ['s', 's', 's'],
                            _L: ['s', 's', 's', 's'], _T: ['g', 'f', 'f'], _Ta: ['g', 'f', 'f'], _Th: ['g', 's', 's'],
                            'Y2F': ['f', 'f'], 'Y2Fa': ['f', 'f'], 'Yab2S': ['s', 's'], 'Chain2Y': ['s', 's', 'f', 'f'],
                            'Chain2Ya': ['s', 's', 'f', 'f'], 'Chain3Y': ['s', 's', 's', 'f', 'f'],
                            'Chain3Ya': ['s', 's', 's', 'f', 'f'], 'Chain4Y': ['s', 's', 's', 's', 'f', 'f'],
                            'Chain4Ya': ['s', 's', 's', 's', 'f', 'f'], 'Chain5Y': ['s', 's', 's', 's', 's', 'f', 'f'],
                            'Chain5Ya': ['s', 's', 's', 's', 's', 'f', 'f'], 'Hbar2abS': ['s', 's'],
                            'H2abS': ['s', 's'], 'Y2FabS': ['g', 's', 's'], 'Y2FabSkin': ['s', 's'],
                            'Habcd': ['s', 's', 's', 's'], 'Theta4': ['g', 'g', 's', 's', 's', 's'],
                            'Theta2': ['g', 's', 's', 's', 's'], 'L2abS': ['s', 's'], 'ffabc': ['g'], 'Hab': ['s', 's'],
                            'Habc': ['s', 's', 's'], 'WW_T': ['f'], _Ckin: ['f'], _Ckins: ['s'], _SfCkinf: ['f'],
    ...
    def __repr__(self):
        ...
        return "This is an instance of the {} model, written by {} on {},\n\t\t Gauge group : {},\n\t\t Contains the different particles :\n\t\tFermions: {}\n\t\tScalars: {}\nPotential :{}\n".format(
            self._Name, self._Author, self._date, self.GaugeGroups, self.Fermions, self.Scalars, self.Potential)
        ...
        for key, val in self.CplxScalars.items():
            for ir, real in enumerate(val.RealComponents):
                if real.args != ():
                    ...
                    if kkey == []:
                        loggingCritical(
                            "error one of the real component is not define in the RealScalars, {}".format(key),
                            verbose=True)
                        exit()
                    else:
                        kkey = kkey[0]
    ...
    def get_different_gaugegroups(self):
        ...
        for name, g, t in self.GaugeGroups:
            if not (t):
                ...
                self.UGaugeGroups.append([name, g, t])
    ...
    def ApplyCGC(self, Term, combination):
        if type(combination[0]) == tuple:
            combout = [list(ell) for ell in combination]
        else:
            combout = [
                list(combination)]  # in the case there is only one gauge group in the whole contractio it s a tuple
        TempGroup = []
    ...
    def GetCombInd(self, CombInd, STOP, Term, counter, Full=False):
        All = False
        tempF = []
        while True:
            try:
                ...
                res = self.ApplyCGC(Term, tempcomb)
                if res != 0:
                    tempF = [tempcomb, res]
                    STOP = True
                    break
    ...
    def ConstructReturn(self, el, tempF, Term, Return, ReturnToCalc):
        ...
        if term == 'Yukawas' or term == 'FermionMasses':
            for iterm, (termkey, termval) in enumerate(self.Potential[term].items()):
                ...
                if type(termval['Fields'][0]) == list:
                    ...
                    scpos = [[il for il, subll in enumerate(ll) if not (subll in ferms[ill])] for ill, ll in
                             enumerate(termval['Fields'])]
                    outfields = [[''] * 3 for ii in range(len(ferms))] if term == 'Yukawas' else [[''] * 2 for ii in
                                                                                                  range(len(ferms))]
                    if scpos != []:
        ...
        for iii, (el, val) in enumerate(self.Potential[term].items()):
            ...
            if type(val['Fields'][0]) == list:
                if term == 'Yukawas' or term == 'ScalarMasses' or term == 'FermionMasses':
                    self.DimYuk[el] = [self.Particles[elem].Gen for elem in val['Fields'][0] if
                                       elem in self.Fermions and self.Particles[elem].Gen != 1]
                # get the quantum numbers
                Ppi = [[self.Particles[subvalfields] for subvalfields in valfields] for valfields in val['Fields']]
            else:
                if term == 'Yukawas' or term == 'ScalarMasses' or term == 'FermionMasses':
                    self.DimYuk[el] = [self.Particles[elem].Gen for elem in val['Fields'] if
                                       elem in self.Fermions and self.Particles[elem].Gen != 1]
                # get the quantum numbers
                    ...
                    if not (all([cc == (0,) for cc in Factor])):
                        ...
                        for idd, fac in enumerate(Factor):
                            if fac != (0,):
                                ...
                                indicesff[idd] = tuple([str(part.indices[idd]).replace('j', 'i') for part in subll[0] if
                                                        part.indices[idd] != 0])
            ...
            if self.MixingTerm[term][el] != []:
                ...
                loggingInfo("Warning, some terms are mixing with each other, trying to disentangle them ...",
                            verbose=True)
                SKIP = True
            ...
            if not (SKIP):
                if Factor != [] and not (FullSinglet):
                    ...
                    while not (STOP):
                        ...
                        CombInd = itr.product(
                            *[itr.product(*[range(1, x[1] + 1) for idx, x in enumerate(element)]) for ielem, element in
                              enumerate(Return[el][counter][1])])
                        tempF, STOP, counter = self.GetCombInd(CombInd, STOP, Return[el][counter], counter)
                    if tempF == []:
                        loggingCritical(
                            "Error no isospin contraction leading to non zero term were found in {}".format(el),
                            verbose=True)
                        exit()
                    else:
                        LHfactor, outparticles = self.ConstructReturn(el, tempF, Return[el][counter], Return,
                                                                      ReturnToCalc)
                elif Factor == [] or FullSinglet:
                    # full singlet
                    counter = 0
                    LHfactor = Return[el][counter][-1]
                    outparticles = [[Symbol(str(ell.args[0]))] + len(self.NonUGaugeGroups) * [0] for ell in
                                    Return[el][counter][0]]
                ...
                try:
                    while not (Found):
                        ...
                        while not (STOP1):
                            if counter == len(Return[el]):
                                loggingCritical(
                                    "WARNING, impossible to disentangle the term: {} calculating a combination".format(
                                        el), verbose=True)
                                ...
                                break
                            CombInd = itr.product(
                                *[itr.product(*[range(1, x[1] + 1) for idx, x in enumerate(element)]) for ielem, element
                                  in enumerate(Return[el][counter][1])])
                            # save cterm
                            STOP2 = False
                            while not (STOP2):
                                tempF, STOP1, counter, All = self.GetCombInd(CombInd, STOP1, Return[el][counter],
                                                                             counter,
                                                                             Full=True)  # all tells us if all the combinations have been tried
                                ...
                                if tempF != []:
                                    ...
                                    for mixwith in self.MixingTerm[term][el]:
                                        ...
                                        res = self.ApplyCGC(TermIdentified, tuple([tuple(lll) for lll in NewInd]))
                                        if res != 0:
    ...
    def ValidateSettings(self, Settings):
        ...
        for part, val in TempParticles.items():
            ...
            if not (Tags):
                loggingCritical("Error, the particle {} is charged under an unknown gauge group.".format(part),
                                verbose=True)
                exit()
            ...
            if key == 'QuarticTerms' or key == 'ScalarMasses' or key == 'TrilinearTerms':
                for qq, xpr in term.items():
                    ...
                    for subxpr in templist:
                        ...
                        if not (Tags):
                            loggingCritical(
                                "Error, the {} {} does not contain only scalars or the wrong number".format(key,
                                                                                                            subxpr),
                                verbose=True)
                            exit()
            if key == 'Yukawas':
                for yy, xpr in term.items():
                    ...
                    for subxpr in templist:
                        ...
                        if not (Tags):
                            loggingCritical(
                                "Error, the yukawa term {} does not contain two fermions and a scalar: {}".format(yy,
                                                                                                                  subxpr),
                                verbose=True)
                            exit()
            if key == 'FermionMasses':
                for ff, xpr in term.items():
                    ...
                    for subxpr in templist:
                        ...
                        if not (Tags):
                            loggingCritical(
                                "Error, the FermionMasses {} does not contain two fermions : {}".format(ff, subxpr),
                                verbose=True)
                            exit()
                ...
                if iff != igg:
                    ...
                    for ffelem in ff:
                        if ffelem in lcFields:
                            ...
                            break
                        if lcFields == []:
                            self.MixingTerm[term][self.Potential[term].keys()[iff]].append(
                                self.Potential[term].keys()[igg])
        return self.MixingTerm

    def ExpandPotential(self, fields, nbfields, term, AnomalousDim=False):
        ...
        for idelem, elemfields in enumerate(fields):
            ...
            for iff, f in enumerate(elemfields):
                ...
                if f.Cplx and not (Singlet):
                    Out.append([IndexedBase(f.RealPart)[
                                    ['i{}{}'.format(iff + 1, igg) if f.Qnb[g[0]] != g[1].Dynksinglet else Integer(0) for
                                     igg, g in enumerate(self.NonUGaugeGroups)]] + f.Coeff * IndexedBase(f.CplxPart)[
                                    ['i{}{}'.format(iff + 1, igg) if f.Qnb[g[0]] != g[1].Dynksinglet else Integer(0) for
                                     igg, g in enumerate(self.NonUGaugeGroups)]], f.norm])
                    ToDerive.append([IndexedBase(f.RealPart)[[
                        'j{}{}'.format(iff + 1, igg) if f.Qnb[g[0]] != g[1].Dynksinglet else Integer(0) for igg, g in
                        enumerate(self.NonUGaugeGroups)]], IndexedBase(f.CplxPart)[[
                        'j{}{}'.format(iff + 1, igg) if f.Qnb[g[0]] != g[1].Dynksinglet else Integer(0) for igg, g in
                        enumerate(self.NonUGaugeGroups)]]])
                elif f.Cplx and Singlet:
                    Out.append([IndexedBase(f.RealPart)[[Symbol('dumi{}{}'.format(iff + 1, igg)) for igg, g in
                                                         enumerate(self.NonUGaugeGroups)]] + f.Coeff *
                                IndexedBase(f.CplxPart)[[Symbol('dumi{}{}'.format(iff + 1, igg)) for igg, g in
                                                         enumerate(self.NonUGaugeGroups)]], f.norm])
                    ToDerive.append([IndexedBase(f.RealPart)[[Symbol('dumj{}{}'.format(iff + 1, igg)) for igg, g in
                                                              enumerate(self.NonUGaugeGroups)]],
                                     IndexedBase(f.CplxPart)[[Symbol('dumj{}{}'.format(iff + 1, igg)) for igg, g in
                                                              enumerate(self.NonUGaugeGroups)]]])
                elif not (f.Cplx) and not (Singlet):
                    Out.append(IndexedBase(f._name)[
                                   ['i{}{}'.format(iff + 1, igg) if f.Qnb[g[0]] != g[1].Dynksinglet else Integer(0) for
                                    igg, g in enumerate(self.NonUGaugeGroups)]])
                    ToDerive.append([IndexedBase(f._name)[[
                        'j{}{}'.format(iff + 1, igg) if f.Qnb[g[0]] != g[1].Dynksinglet else Integer(0) for igg, g in
                        enumerate(self.NonUGaugeGroups)]]])
                else:
                    Out.append(IndexedBase(f._name)[[Symbol('dumi{}{}'.format(iff + 1, igg)) for igg, g in
                                                     enumerate(self.NonUGaugeGroups)]])
                    ToDerive.append([IndexedBase(f._name)[[Symbol('dumj{}{}'.format(iff + 1, igg)) for igg, g in
                                                           enumerate(self.NonUGaugeGroups)]]])
            ...
            Out = (self.translatenorm(lcNorm[idelem]) * functools.reduce(operator.mul, [
            ...
            CollectDummy = [[(el, Integer(0)) for el in elem[1].atoms() if len(str(el).split('dum')) != 1] for elem in
                            FOut]
            SumFOut.append(
                [[tuple([xx.subs(tuple(CollectDummy[iel])) for xx in el[0]]), el[1].subs(tuple(CollectDummy[iel]))] for
                 iel, el in enumerate(FOut)])
        return SumFOut
    ...
    def Expand2(self, args, dotrace=True, Layer=0, MatStruc=[]):
        ...
        for ielem, elem in enumerate(Blocks):
            if type(elem[0]) == list:  # sum of terms one more layer to go down
                SaveChain.append(sum([list(el[1:]) for el in elem[0]],
                                     []))  # the structure is the same in all the elements of one block no need to go through
        ...
        for icm, combo in enumerate(it):
            ...
            indicesFull = [indices[el[1]] for el in
                           GeneratePosition]  # position in Generate are given with respect to the Chain list :w
                ...
                if type(block[0]) == list:  # we are dealing with a sum of terms
                    ...
                    for iee, element in enumerate(block):  # these terms are multiplied together
                        ...
                        if iee != 0:
                            ...
                            cpGenIndices = [GenIndices[i] for i in BlockMapping[ib][iee]] + GenIndices[
                                                                                            LinkSumSaveChain[ib]:]
                            cpindicesFull = [indicesFull[i] for i in BlockMapping[ib][iee]] + indicesFull[
                                                                                              LinkSumSaveChain[ib]:]
                        else:
                        ...
                        for term in element:
                            if term == _Th or term == _T or term == _Ta or term == _Tkin or term == _Takin or term == _TakinT or term == _TkinT:
            ...
            if Out != [] and not (STOP):
                ...
                if Traces != (None, None):
                    ...
                    if type(InTrace) == Add:
                        ...
                        for el in InTS:
                            InTF = InTF.subs(el, 1)
                        Out = functools.reduce(operator.mul, OutOfTrace, 1) * InTF * Tr(
                            functools.reduce(operator.mul, InTS))
                else:
                    Out = functools.reduce(operator.mul, Out, 1)
                listsum = [[ll for ll in el if len(str(ll[0]).split('dum')) != 2
                            and len(str(ll[1]).split('dum')) != 2] for el in
                           SummedIndices]  # got rid of the dummy indices
    ...
    def Expand(self, args, dotrace=True, Layer=0, MatStruc=[]):
        ...
        for icm, combo in enumerate(it):
            ...
            # duplicate the indices that are repeated
            indicesFull = [indices[el[1]] for el in
                           GeneratePosition]  # position in Generate are given with respect to the Chain list 
            # SummedIndices = list(indices)
            SummedIndices = copy.deepcopy(indices)
            # add the ones that are fixed in the correct place
            for el in FixedIndices:
            ...
            for itp, tp in enumerate(TChain):
                ...
                if tp == _Y:
                    ...
                    STOP, temp, indicesFull = tempupdate(3, temp,
                                                         indicesFull)  # update teh different dictionary and break if True is returned
                elif tp == _Ya:
                    ...
                    STOP, temp, indicesFull = tempupdate(1, temp, indicesFull)
                ################################
                # New terms for kinetic mixing. F. 25.07.14
                ################################
                elif tp == 'WW_T':
                    res = self.WW_T(temp[:1])
                    STOP, temp, indicesFull = tempupdate(1, temp, indicesFull)
                elif tp == 'Y2FabSkin':
                    res = self.Y2FabSkin(temp[:2], indicesFull[:2])
                    STOP, temp, indicesFull = tempupdate(2, temp, indicesFull)
                elif tp == _Ckin or tp == _Ckins:
                    res = self.Casimirkin(temp[:1])
                    STOP, temp, indicesFull = tempupdate(1, temp, indicesFull)
                elif tp == _SfCkinf or tp == _SfCkins:
                    res = self.DynkinCasimirkin(temp[:1], fermion=True)
                    STOP, temp, indicesFull = tempupdate(1, temp, indicesFull)
                elif tp == _SsCkins or tp == _SsCkinf:
                    res = self.DynkinCasimirkin(temp[:1], fermion=False)
                    STOP, temp, indicesFull = tempupdate(1, temp, indicesFull)
                elif tp == _Takin:
                    res = self.TMatkin(temp[:1], adj=True, tp=False)
                    STOP, temp, indicesFull = tempupdate(1, temp, indicesFull)
                elif tp == _Tkin:
                    res = self.TMatkin(temp[:1], adj=False, tp=False)
                    STOP, temp, indicesFull = tempupdate(1, temp, indicesFull)
                elif tp == _TakinT:
                    res = self.TMatkin(temp[:1], adj=True, tp=True)
                    STOP, temp, indicesFull = tempupdate(1, temp, indicesFull)
                elif tp == _TkinT:
                    ...
                    res = self.Thetakin4W(temp[:4], indicesFull[:4])
                    STOP, temp, indicesFull = tempupdate(4, temp, indicesFull)
            ...
            if Out != [] and not (STOP):
                ...
                if Traces != (None, None):
                    ...
                    Out = functools.reduce(operator.mul, Out, 1)
                    # Scalar product when kinetic mixing is on
                    if type(Out) == tMM and Out.shape == (1, 1):
                        Out = Out[0]
                listsum = [[ll for ll in el if len(str(ll[0]).split('dum')) != 2
                            and len(str(ll[1]).split('dum')) != 2] for el in
                           SummedIndices]  # got rid of the dummy indices
                ...
                if ExtraConstrains != []:
                    constrains = self.applyExtraConstrains2(ExtraConstrains, MatStruc)
                # multiply Out by the constrains
                # Here we also need to switch on the type of Out for the kinetic micxing in case we deal with matrices
                if type(Out) == tMM:
                    Out = summatrix([Out * llcons for llcons in constrains])
                else:
                    Out = sum([Out * llcons for llcons in constrains])
                # create the sum out of this 
                argsum = [str(ll) for ll in listsum]
                if argsum != []:
                    # Here we need some modifications for the kinetic mixing
                    if type(Out) == tMM:
                        for iel, el in enumerate(Out):
                            Out[iel] = eval("Sum(el," + ','.join(argsum) + ")")
                            if Out[iel] != 0:
    ...
    def applyExtraConstrains2(self, ExtraConstrains, MatStruc):
        ...
        ListKronecker = []
        for ll in MatStruc:
            Doubles = [[(ielem, iel + ielem + 1) for iel, el in enumerate(ll[ielem + 1:]) if el == elem] for ielem, elem
                       in enumerate(ll)]
            Doubles = sum((Doubles), [])
            # construct kronecker for the required indices
            GroupConstrains = [el[0].split('A')[-1] if el != [] and len(el[0].split('fabc')) == 1 else (
                el[0].split('fabc')[-1] if el != [] and len(el[0].split('fabc')) != 1 else 1) for el in ExtraConstrains]
            Constrains = functools.reduce(operator.mul, [KroneckerDelta(ExtraConstrains[i][0], ExtraConstrains[j][0])
                                                         for (i, j) in Doubles if
                                                         ExtraConstrains[i] != [] and ExtraConstrains[j] != []], 1)
    ...
    def getcoeffterm(self, temp, indicesFull, tp):
        ...
        ###########
            ...
            res = self.Casimirkin(temp[:1])
            return [res, 1]
        elif tp == _SfCkins or tp == _SfCkinf:
            res = self.DynkinCasimirkin(temp[:1], fermion=True)
            return [res, 1]
        elif tp == _SsCkins or tp == _SsCkinf:
            res = self.DynkinCasimirkin(temp[:1], fermion=False)
    ...
    def DeterminTraces2(self, argsS, Structure):
        ...
        for ielem, elem in enumerate(argsS):
            ...
            ###########
                ...
                StartingV = elem[1]
            elif elem[0] == _Ckin or elem[0] == _Ckins or elem[0] == _SfCkinf or elem[0] == _SfCkins or elem[
                0] == _SsCkinf or elem[0] == _SsCkins:
                StartingV = elem[1]
            elif elem[0] == 'Y2FabSkin':
                StartingV = None
            elif elem[0] == _Tkin or elem[0] == _Takin or elem[0] == _TakinT or elem[0] == _TkinT:
                StartingV = elem[1]
            else:
                exit("Determin Trace2 : {} not implemented".format(elem[0]))
            ...
            if StartingV != None:
                ...
                for ix, x in enumerate(argsS[ielem + 1:]):
                    ...
                    if StartingV in FinalV:
                        ...
                        InTrace = [iel for iel, el in enumerate(argsS) if
                                   iel in range(ielem + iel4Y, ix + ielem + 1 + 1)]
                        if elem[0] == _C or elem[0] == _Ckin or elem[0] == _Ckins or elem[0] == _SfCkinf or elem[
                            0] == _SfCkins or elem[0] == 'WW_T' or elem[0] == _SsCkinf or elem[
                            0] == _SsCkins:  # we have to subtract one since C is a number multiplied out already 
    ...
    def DeterminTraces(self, args):
        ...
        for ielem, elem in enumerate(args):
            if elem[0] == _Y or elem[0] == _Ya:
                ...
                exit("Determin Trace: {} not implemented".format(elem[0]))
            # Get all the final values
            iel4Y = 0
            if StartingV != None:
                FinalV = []
                ...
                for ix, x in enumerate(args[ielem + 1:]):
                    ...
                    if StartingV in FinalV:
                        # we found one let's get it
                        InTrace = [iel for iel, el in enumerate(args) if
                                   iel in range(ielem + iel4Y, ix + ielem + 1 + 1)]
                        if elem[0] == _C or elem[0] == _Ckin or elem[0] == _Ckins or elem[0] == _SfCkins or elem[
                            0] == _SfCkinf or elem[0] == _SsCkins or elem[
                            0] == _SsCkinf:  # we have to remove the _C index since it is transformed as a number when called it is not in the trace
                            InTrace.remove(ielem)
                        elif elem[0] == 'WW_T':
                            InTrace.remove(ielem)
                        return InTrace
            ...
            if TermIdentified == []:
                ...
                for tterm in TermIdentified:
                    ...
                    if term == 'Yukawas' or term == 'FermionMasses':
                        ...
                        if Adj:
                            ToDatabased.append((self.Classes[tterm[0]](Symbol(tterm[0], commutative=False),
                                                                       parts[i0 + 1], parts[i0]), Factor, Adj))
                        else:
    ...
    def DynkinCasimirkin(self, label, fermion=False):
        ...
        key = tuple(['SfCkin'] + label) if fermion else tuple(['SsCkin'] + label)
        if key in self.InvariantResults:
            return self.InvariantResults[key]
        else:
            if fermion:
                res = sum(flatten(
                    [multiplicityKin(pp, self) * (pp.W.transpose() * self.Particles[str(label[0])].W) ** 2 for pp in
                     self.Fermions.values()]))
            else:
                res = sum(flatten(
                    [multiplicityKin(pp, self) * (pp.W.transpose() * self.Particles[str(label[0])].W) ** 2 for pp in
                     self.Scalars.values()]))
        ...
        if key in self.InvariantResults:
            ...
            if Inter:
                if not (grp.U):
                    ...
                    f1 = [self.ScalarsIndex[label[1]]] + [self.Particles[str(label[1])].Qnb[label[0]]] + [ind for
                                                                                                          idi, ind in
                                                                                                          enumerate(
                                                                                                              indices[
                                                                                                                  1]) if
                                                                                                          idi == idgr]
                    f2 = [self.ScalarsIndex[label[2]]] + [self.Particles[str(label[2])].Qnb[label[0]]] + [ind for
                                                                                                          idi, ind in
                                                                                                          enumerate(
                                                                                                              indices[
                                                                                                                  2]) if
                    ...
                    if len(str(f1[-1][0]).split('dum')) == 2:  # particles are not charged under the group
                        ...
                        res = Ts(Indx[0], grp.MatRepr(f1, f2, HB=True), f1[-1][0],
                                 f2[-1][0]) * Kr  # take the indices label for f1 and f2
                        res = res if not (adj) else res.conjugate()
                        self.InvariantResults[key] = res, Indx
                        return res, Indx
                else:  # U1 group
                    if not (self.kinmixing):
                        ...
                        # Get the Hypercharge
                        f1h = self.Particles[str(label[1])].Qnb[
                            label[0]]  # f2h is the same condition enforced at the begining
                        f1, f2 = [self.ScalarsIndex[label[1]], f1h] + f1ind, [self.ScalarsIndex[label[2]], f1h] + f2ind
                        res = grp.MatRepr(f1, f2,
                                          HB=True)  # no need for symbolic function here everything is known and nothing summed easy !
                        res = res if not (adj) else res.conjugate()
                        self.InvariantResults[key] = res, []
                        return res, []
                    else:
    ...
    def TMatRepr(self, label, indices, indlabel, adj=False):
        ...
        key = tuple(['TMat'] + label + flatten(indices) + [indlabel]) if not (adj) else tuple(
            ['TaMat'] + label + flatten(indices) + [indlabel])
        if key in self.InvariantResults:
            return self.InvariantResults[key]
        else:
            if label[1] == label[-1]:  # They have to come from the same particle
                ...
                if not (grp.U):
                    ...
                    f1 = [self.Particles[str(label[1])].Qnb[label[0]]] + [ind for idi, ind in enumerate(indices[1]) if
                                                                          idi == idgr]
                    f2 = [self.Particles[str(label[2])].Qnb[label[0]]] + [ind for idi, ind in enumerate(indices[2]) if
                                                                          idi == idgr]
                    ...
                    if len(str(f1[-1][0]).split('dum')) == 2:  # particles are not charged under the group
                        ...
                        Ind = [[f1kr[i][0], f2kr[i][0]] for i in range(len(f1kr)) if
                               len(str(f1kr[i][0]).split('dum')) != 2]
                        Kr = functools.reduce(operator.mul, [KroneckerDelta(i, j) for (i, j) in Ind], 1)
                        res = Ts(Indx[0], grp.MatRepr(f1[0], f2[0]), f1[-1][0],
                                 f2[-1][0]) * Kr  # take the indices label for f1 and f2
                        res = res if not (adj) else res.conjugate()
                        self.InvariantResults[key] = res, Indx
                        return res, Indx
                    ...
                    if not (self.kinmixing):
                        ...
                        res = grp.MatRepr(f1,
                                          f2)  # no need for symbolic function here everything is known and nothing summed easy !
                        res = res if not (adj) else res.conjugate()
                        self.InvariantResults[key] = res, []
                        return res, []
                    else:
    ...
    def Y2FabSkin(self, parts, indices):
        ...
        if key in self.InvariantResults:
            ...
            res = Rational(1, 2) * (
                self.Expand(((_Ckin, p1), (_Ya, sc1, p1, p2), (_Y, sc2, p2, p1)), Layer=1)
                + self.Expand(((_Ckin, p1), (_Ya, sc2, p1, p2), (_Y, sc1, p2, p1)), Layer=1)
            )
            if res != 0:
                res = res.doit()
            self.InvariantResults[key] = res
    ...
    def TMatkin(self, label, adj=False, tp=False):
        ...
        if tp:
            res = self.Fermions[str(label[0])].W.transpose()
        else:
            res = self.Fermions[str(label[0])].W
        if adj:
            # res = res.adjoint()
            pass
        return res
    ...
    def Thetakin4W(self, parts, indices):
        ...
        if key in self.InvariantResults:
            ...
            sc1, p1, sc2, p2 = parts
            res = self.Scalars[str(sc1)].W.transpose() * self.Fermions[str(p1)].W * self.Scalars[
                str(sc2)].W.transpose() * self.Fermions[str(p2)].W
            res = sum(res)
            self.InvariantResults[key] = res
        return res
    ...
    def ThetakinWsWs(self, parts, indices):
        ...
        if key in self.InvariantResults:
            ...
            sc1, sc2 = parts
            res = self.Scalars[str(sc1)].W.transpose() * self.Scalars[str(sc2)].W
            res = sum(res)
            self.InvariantResults[key] = res
        return res
    ...
    def Habc(self, parts, indices):
        ...
        if key in self.InvariantResults:
            ...
            res = [Rational(1, 2) * (
                self.Expand(((_mf, p1, p2), (_Ya, a, p2, p3), (_Y, b, p3, p4), (_Ya, c, p4, p1)), Layer=1)
    ...
    def Hab(self, parts, indices):
        ...
        if key in self.InvariantResults:
            ...
            #		)
            res = (self.Expand2((([[(_Y, sc1, p1, p2), (_Ya, sc2, p2, p3)], [(_Y, sc2, p1, p2), (_Ya, sc1, p2, p3)]]),
                                 (_mf, p3, p4), (_mfa, p4, p1)), Layer=1)  # Line 1
                   + self.Expand2((([[(_Ya, sc1, p1, p2), (_Y, sc2, p2, p3)], [(_Ya, sc2, p1, p2), (_Y, sc1, p2, p3)]]),
                                   (_mfa, p3, p4), (_mf, p4, p1)), Layer=1)  # Line 2
                   + self.Expand(((_Y, sc1, p1, p2), (_mfa, p2, p3), (_Y, sc2, p3, p4), (_mfa, p4, p1)), Layer=1)
                   + self.Expand(((_mf, p1, p2), (_Ya, sc1, p2, p3), (_mf, p3, p4), (_Ya, sc2, p4, p1)), Layer=1)
                   )
            if res != 0:
    ...
    def Theta4(self, parts, indices):
        ...
        if key in self.InvariantResults:
            ...
            res = (
                self.Expand(((_Th, g1, sc1, s1), (_Th, g1, sc3, s2), (_Th, g2, s1, sc2), (_Th, g2, s2, sc4)), Layer=1,
                            MatStruc=['A', 'A', 'B', 'B'])
    ...
    def Chain3Yuk(self, parts, indices, adj=False):
        ...
        key = tuple(['Chain3Y'] + parts + flatten(indices)) if not (adj) else tuple(
    ...
    def Chain5Yuk(self, parts, indices, adj=False):
        ...
        if key in self.InvariantResults:
            ...
            if not (adj):
                ...
                res = self.Expand((('Chain4Ya', sc1, sc2, sc3, sc4, f1, p1), (_Ya, sc5, p1, f2)), Layer=3,
                                  dotrace=False)
            if res != 0:
                res = res.doit()
            self.InvariantResults[key] = res
    ...
    def declarations(self):
        ...
        self.declareSymbol(
            ['_Y', '_Ya', '_C', '_Cs', '_S', '_Ss', '_Cg', '_G', '_mf', '_mfa', '_ms', '_L', '_h', '_Tha', '_Ta', '_Th',
             '_T', '_Ckin', '_Ckins', '_SfCkinf', '_SfCkins', '_SsCkinf', '_SsCkins', '_TkinT', '_TakinT', '_Tkin',
             '_Takin', 'l1', 'l2', 'l3', 'l4', 'l5', 'l6', 'l7', 'l8', 'l9', 'l10', 'l100', 'l101', 'l11', 'l12', 'l13',
             'l131', 'l132', 'l14', 'l15', 'l16', 'l17', 'l140', 'l141', 'l142', 'l143', 'l150', 'l151',
             'l152'])  # the last symbls is for the C2(G)
        ...
        if type(part) == str and part in self.mod.GaugeGroupsName:
            ...
            for ig, group in enumerate(self.mod.NonUGaugeGroups):
                if part[ig + 1] != 0 or type(part[ig + 1]) == Symbol:  # the particle is charged
                    if len(str(part[ig + 1]).split(
                            'dum')) == 2:  # check if the ext ind is map to a dummy one in this case keep dumm
