...
def multiplicity(x, part, val, Mod):
    ...
    Multiplicity = [el if el != 0 else 1 for el in
                    Multiplicity]  # when the dim is zero i.e. singlet the multiplicity should be one
                ...
                for el in lOutorIn:
                    ...
                    if type(el) != adjoint:
                        ...
                        if hasattr(el.args[0], 'arg'):
                            Transpose = False if el.args[0].arg == model.DefOrderYuk[str(el.args[0].Symb)][
                                                                   ::-1] else True
                        Toadd = el.args[0].Symb.conjugate() if Transpose else el.args[
                            0].Symb.adjoint()  # we apply transpose on the adjoint
                    else:
                    ...
                    while True:
                        ...
                        if len(Term) == 1:
                            ...
                            skip = [model.Classes[str(el[0])](el[1]) if str(el[0]) in model.Classes and el[1] != () else
                                    (Symbol(str(el[0].args[0]), commutative=True).conjugate() if type(
                                        el[0]) == conjugate and el[1] == () else (
                            ...
                            if str(Term[0]) in model.Classes:
                                if (len(Indices) == 1 and Indices[0] == ()) or Indices == []:
                                    ...
                                    if len(Indices) == 1 and skip != [] and type(skip[0]) == MatM:
                                        if Indices[-1][0] == Indices[-1][1] and len(
