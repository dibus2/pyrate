...
parser.add_argument('--ScalarAnomalous', '-sca', dest='ScalarAnomalous', action='store_true', default=False,
                    help='set the calculation of scalar anomalous dimensions to True')
parser.add_argument('--FermionAnomalous', '-fera', dest='FermionAnomalous', action='store_true', default=False,
...
if RunSettings['Model'] == "":
    ...
    if RunSettings['All-Contributions']:
        ...
        loggingInfo("\t\t... with option --All-Contributions including{} ".format(strListLagrangian),
                    verbose=RunSettings['vInfo'])
        ToCalculate = ['Gauge-Couplings'] + [Translation[el] for el in ListLagrangian] + ['ScalarAnomalous'] + [
            'FermionAnomalous']
    else:
