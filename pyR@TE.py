...
welcomemessage = """\n\n\n\n\t\t==================================================================================\n
\t\t\t\tPyR@TE version 1.1.1 released  2014\n
...
parser.add_argument('--Model','-m', dest='Model', action= 'store', default = '',help="This option is used to specify the YAML file containing the model settings")
parser.add_argument('--verbose','-v', dest ='verbose', action = 'store_true',  default = False, help='Prints details on the calculation')
parser.add_argument('--VerboseLevel','-vL', dest ='VerboseLevel', action = 'store',  default = '', help='Set up the level of printing')
parser.add_argument('--Gauge-Couplings','-gc', dest ='Gauge-Couplings', action = 'store_true',  default = False, help='Set the calculation to the gauge couplings beta functions')
parser.add_argument('--Quartic-Couplings','-qc', dest ='Quartic-Couplings', action = 'store_true',  default = False, help='Set the calculation to the quartic couplings beta functions')
parser.add_argument('--Yukawas','-yu', dest ='Yukawas', action = 'store_true',  default = False, help='Set the calculation to the yukawas beta functions')
parser.add_argument('--ScalarMass','-sm', dest ='ScalarMass', action = 'store_true',  default = False, help='Set the calculation to the Scalar Mass beta functions')
parser.add_argument('--FermionMass','-fm', dest ='FermionMass', action = 'store_true',  default = False, help='Set the calculation to the Fermion Mass beta functions')
parser.add_argument('--Trilinear','-tr', dest ='Trilinear', action = 'store_true',  default = False, help='Set the calculation to the Trilinear term beta functions')
...
parser.add_argument('--Pickle','-pkl', dest ='Pickle', action = 'store_true',  default = False , help='Set if a pickle output should be produced')
parser.add_argument('--PickleFile','-pf', dest ='PickleFile', action = 'store',  default = 'RGEsOutput.pickle' , help='Set the name of the pickle output file')
parser.add_argument('--TotxtMathematica','-tm', dest ='ToM', action = 'store_true',  default = False , help='Set if a pickle output should be produced')
parser.add_argument('--TotxtMathFile','-tmf', dest ='ToMF', action = 'store',  default = 'RGEsOutput.txt' , help='Set the name of the txt to mathematica file')
parser.add_argument('--Weyl','-w', dest = 'Weyl', action= 'store', default = True, help='Set if the fields are weyl or dirac, i.e. if kappa should be taken equal to 1 or 1/2')
parser.add_argument('--Export','-e',dest = 'Export', action= 'store_true', default = False, help = 'Set if a python function is created to evaluate the beta function')
	...
	if RunSettings['All-Contributions'] :
		...
		if RunSettings['ScalarAnomalous']:
			if model.ScalarAnomalousToCalculate == {} :
				loggingCritical("\t\t**WARNING**, No Scalar Anomalous dimension found to calculate. Each anomalous is associated to a scalar mass term, make sure that the corresponding scalar mass terms are defined in the potential.", verbose=True)
			else :
