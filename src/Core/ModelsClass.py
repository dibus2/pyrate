...
class Model(object):
    ...
    def get_different_gaugegroups(self):
        ...
        for GroupName, group in self.GaugeGroupsSettings.items():
            ...
            if group == 'U1':
                ...
                self.GaugeGroups.append([GroupName, SOn(int(group[2:]), GroupName, self.idb), False])
            else:
        ...
        if type(normdic) != str:
            ...
            if ('I' in normdic or 'sqrt' in normdic) and not ('**' in normdic):
                ...
                normdic = eval(normdic.replace('{}/{}'.format(src.group(1), src.group(2)),
                                               '(Rational({},{}))'.format(src.group(1), src.group(2))))
                return normdic
            else:
                try:
                            ...
                            while not (STOP2):
                                ...
                                if tempF != []:
                                    ...
                                    for mixwith in self.MixingTerm[term][el]:
                                        ...
                                        Structureindices = [[elem[i] for elem in tempstructureindices] for i in
                                                            range(len(self.NonUGaugeGroups))]
                                        ...
                                        Structureindices = [elstruc for elstruc in Structureindices if
                                                            not (all([elemstruc == 0 for elemstruc in elstruc]))]
                                        ...
                                        while True:
                                            ...
                                            if lctterm == lcparts:
                                                ...
                                                pass
                                                # Get rid of the zeros againF.:This I copied from online
    ...
    def ExpandPotential(self, fields, nbfields, term, AnomalousDim=False):
        ...
        for idelem, elemfields in enumerate(fields):
            ...
            #        tempC = tempC + [f.Qnb[gg[0]] for gg in self.UGaugeGroups]
                    ...
                    Out.append([IndexedBase(f.RealPart)[[Symbol('dumi{}{}'.format(dummyff + 1, igg)) for igg, g in
                                                         enumerate(self.NonUGaugeGroups)]] + f.Coeff *
                                IndexedBase(f.CplxPart)[[Symbol('dumi{}{}'.format(dummyff + 1, igg)) for igg, g in
                                                         enumerate(self.NonUGaugeGroups)]], f.norm])
                    ToDerive.append([IndexedBase(f.RealPart)[[Symbol('dumj{}{}'.format(dummyff + 1, igg)) for igg, g in
                                                              enumerate(self.NonUGaugeGroups)]],
                                     IndexedBase(f.CplxPart)[[Symbol('dumj{}{}'.format(dummyff + 1, igg)) for igg, g in
                                                              enumerate(self.NonUGaugeGroups)]]])
                    dummyff += 1
    ...
    def ThetakinC4s(self, parts, indices):
        ...
        if key in self.InvariantResults:
            ...
            res = sum([sum(multiplicityKin(pp, self) * self.Scalars[str(sc1)].W.transpose() * pp.W * pp.W.transpose() * self.Scalars[str(sc3)].W)
